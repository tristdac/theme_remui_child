...
use \theme_remui_child\activity_meta;
...
class activity {
    ...
    public static function module_meta(\cm_info $mod) {
        ...
        if (method_exists('theme_remui_child\\activity', $methodname)) {
            $meta = call_user_func('theme_remui_child\\activity::' . $methodname, $mod);
        } else {
    ...
    protected static function std_meta(\cm_info $mod,
        ...
        $meta->submittedstr = get_string($submitstrkey, 'theme_remui_child');
        $meta->notsubmittedstr = get_string('not'.$submitstrkey, 'theme_remui_child');
        if (get_string_manager()->string_exists($mod->modname.'draft', 'theme_remui_child')) {
            $meta->draftstr = get_string($mod->modname.'draft', 'theme_remui_child');
        } else {
            $meta->drafstr = get_string('draft', 'theme_remui_child');
        }

        if (get_string_manager()->string_exists($mod->modname.'reopened', 'theme_remui_child')) {
            $meta->reopenedstr = get_string($mod->modname.'reopened', 'theme_remui_child');
        } else {
            $meta->reopenedstr = get_string('reopened', 'theme_remui_child');
        }
        ...
        if (has_capability('mod/assign:grade', $mod->context)) {
            ...
            if (method_exists('theme_remui_child\\activity', $methodnsubmissions)) {
                $meta->numsubmissions = call_user_func('theme_remui_child\\activity::'.
                    $methodnsubmissions, $courseid, $mod->instance);
            }
            if (method_exists('theme_remui_child\\activity', $methodnungraded)) {
                $meta->numrequiregrading = call_user_func('theme_remui_child\\activity::'.
